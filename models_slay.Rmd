...
predictions_xgboost <- pred_xgboost_probs
```
...
```{r}
# rf
hist(predictions_random_forest)

#xgboost
hist(pred_xgboost_probs)
...
hist(ensembled.probs$mean_prob)
...
for(col in 1:4){
  ...
  plot(1-specificity_vals, sensitivity_vals, main = paste("Method", col), type = "l",
  ...
  legend_width <- diff(range(specificity_vals)) / 8 
  legend_height <- diff(range(sensitivity_vals)) / 30  
  layout(1)
  layout(matrix(1:2, ncol = 2), widths = c(5, 1), heights = c(1,1))  
  plot(1-specificity_vals, sensitivity_vals, main = col, 
  ...
  par(mar = c(5, 0, 4, 5))  
  image(z = z, col = colors, xaxt = "n", yaxt = "n", bty = "n", xlab = "", ylab = "")
  ...
  auc <- c(auc, sum(diff(specificity_vals) * (sensitivity_vals[-1] + 
